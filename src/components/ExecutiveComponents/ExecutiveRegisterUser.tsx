...
import { UserPlus, Check, AlertCircle, Package as PackageIcon, Coins, Calendar, Sparkles } from "lucide-react";
import React, { useState, useEffect } from "react";

interface ExecutiveRegUserProps {
...
const ExecutiveRegisterUser: React.FC<ExecutiveRegUserProps> = ({
  setOpenUserModel,
  refreshUser,
}) => {
  // Form States
  ...
  const [executiveRefCode, setExecutiveRefCode] = useState("");

  // Get executive's referral code on mount
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (token) {
      try {
        const payload = JSON.parse(atob(token.split('.')[1]));
        setExecutiveRefCode(payload.referralCode || "");
      } catch (err) {
    ...
    try {
      ...
      const response = await axios.post(
        `${BackendUrl}/executive/register-user`,
        {
          ...
          email,
          password,
          executiveRefode: executiveRefCode,
          packageName,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      ...
      <div className="relative w-full max-w-6xl max-h-[90vh] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-gradient-to-tr from-indigo-400/30 to-blue-400/30 rounded-full blur-3xl" />
        ...
        <div className="relative z-10 overflow-y-auto flex-1">
          {!regSuccessful ? (
                  ...
                  {executiveRefCode && (
                    <div className="flex items-center gap-2 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
                      ...
                      <span className="text-sm text-indigo-900">
                        Referral Code: <strong>{executiveRefCode}</strong>
                      </span>
                    </div>
                  )}
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition resize-none"
                      rows={2}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition appearance-none bg-white"
                      required
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold px-6 py-2.5 rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl text-sm"
                  >
                    {loading ? (
                  ...
                  {selectedPackage ? (
                    <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-200 space-y-4">
                      ...
                      <div className="bg-gradient-to-r from-purple-100 to-pink-100 rounded-xl p-4 border border-purple-200">
                        <p className="text-sm text-purple-900 mb-1">Package Price</p>
            ...
            <div className="p-8 text-center space-y-6 max-w-2xl mx-auto">
              ...
              {registrationData && (
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 border border-indigo-200 space-y-3 text-left">
                  <div className="flex items-center justify-between pb-3 border-b border-indigo-200">
                    <span className="text-sm font-medium text-indigo-900">Registration ID</span>
                    <span className="text-sm font-mono text-indigo-700 bg-white px-3 py-1 rounded-lg">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-indigo-900">Submitted At</span>
              ...
              <div className="flex gap-4 justify-center">
                <button
                  ...
                  className="px-6 py-3 bg-white border-2 border-indigo-600 text-indigo-600 font-semibold rounded-xl hover:bg-indigo-50 transition"
                >
...
export default ExecutiveRegisterUser;
