...
const MentorProfileMenu: React.FC<Props> = ({ onSignout }) => {
  ...
  useEffect(() => {
    if (menuOpen && menuRef.current) {
      ...
      if (specializationRef.current) {
        gsap.from(specializationRef.current, {
          opacity: 0,
          y: 20,
          duration: 0.5,
          delay: 0.25,
  ...
  const getJoinedDate = () => {
    if (!mentor?.createdAt) return 'Recently';
    try {
  ...
  const getInitial = () => {
    return mentor?.name?.charAt(0).toUpperCase() || 'M';
  };
  ...
  return (
    <div className="relative">
      {/* Profile Button */}
      <button
        className="w-10 h-10 sm:w-10 sm:h-10 rounded-full bg-green-500 flex items-center justify-center focus:outline-none hover:ring-4 hover:ring-white/30 transition-all duration-200 shadow-lg hover:scale-110"
        onClick={() => setMenuOpen(!menuOpen)}
        aria-label="Mentor profile menu"
      >
        <div 
          ref={avatarRef}
          className="w-full h-full rounded-full bg-gradient-to-br from-teal-400 to-blue-600 flex items-center justify-center text-white text-lg sm:text-xl font-bold"
        >
          {getInitial()}
        </div>
      </button>
      ...
      {menuOpen && (
        <>
          ...
          >
            ...
            >
              {/* Header */}
              <div className="bg-gradient-to-br from-blue-600 via-emerald-600 to-teal-500 px-4 pt-4 pb-4">
                <div ref={headerRef} className="flex items-start gap-3 mb-3">
                  {/* Avatar */}
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 backdrop-blur-sm flex items-center justify-center text-white text-xl font-bold shadow-lg ring-2 ring-white/30">
                    {getInitial()}
                  </div>
                ...
                {mentor?.specialization && (
                  <div ref={specializationRef} className="bg-white/10 backdrop-blur-sm rounded-lg p-2.5 border border-white/20">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white/90 text-xs">Specialization</span>
                      </div>
                      <span className="text-white text-sm font-semibold">{mentor.specialization}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div ref={statsRef} className="px-3 py-3 grid grid-cols-2 gap-2">
                <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-2 hover:scale-105 transition-transform duration-200">
                  <p className="text-[10px] text-green-600 font-medium mb-0.5">Total Webinars</p>
                  <p className="text-lg font-bold text-green-700">{mentor?.totalWebinars || '0'}</p>
                </div>
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-2 hover:scale-105 transition-transform duration-200">
                  <p className="text-[10px] text-blue-600 font-medium mb-0.5">Active</p>
                  <p className="text-lg font-bold text-blue-700">{mentor?.activeWebinars || '0'}</p>
                </div>
              </div>
              ...
              <ul ref={linksRef} className="px-3 py-2 space-y-0.5">
                ...
                <li>
                  <button
                    onClick={() => handleNav("/mentor/my-webinars")}
                    className="w-full flex items-center gap-2.5 py-2 px-2.5 rounded-lg hover:bg-gray-50 text-gray-700 transition-all text-left group"
                  >
                ...
                <li>
                  <button
                    onClick={() => handleNav("/Mentor/Calendar")}
                    className="w-full flex items-center gap-2.5 py-2 px-2.5 rounded-lg hover:bg-gray-50 text-gray-700 transition-all text-left group"
                  >
...
export default MentorProfileMenu;
