...
const DisplayAllProducts: React.FC = () => {
  ...
  const [showSupremePopup, setShowSupremePopup] = useState(false);
  ...
  // Fetch products when filters change - NOW ALWAYS LOADS
  useEffect(() => {
    fetchProducts();
  ...
  const isSupreme = userPackage && userPackage.toLowerCase() === SUPREME_NAME;
  const hasUnlocked = yearsCompleted !== null && yearsCompleted >= REQUIRED_YEARS;
  const yearsLeft =
    yearsCompleted !== null
  ...
  const handleRequestProduct = (product: Product) => {
    // Check if user has Supreme access
    if (!isSupreme && !hasUnlocked) {
      setShowSupremePopup(true);
      return;
    }

    // User is Supreme or unlocked, proceed to request modal
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br border-2 p-2 border-slate-300 rounded-lg from-gray-50 to-gray-100">
      ...
      {/* Supreme Required Popup */}
      {showSupremePopup && (
        <SupremeRequiredPopup
          userPackage={userPackage}
          yearsCompleted={yearsCompleted ?? 0}
          yearsLeft={yearsLeft}
          onClose={() => setShowSupremePopup(false)}
        />
      )}

      {/* Request Modal (only for Supreme users) */}
...
// Supreme Required Popup
interface SupremeRequiredPopupProps {
...
const SupremeRequiredPopup: React.FC<SupremeRequiredPopupProps> = ({
  userPackage,
  yearsCompleted,
  yearsLeft,
  onClose,
  ...
  return (
    ...
    >
      <div
        className="max-w-lg w-full bg-gradient-to-br from-purple-600 via-blue-500 to-yellow-500  rounded-3xl shadow-2xl p-8 relative overflow-hidden"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center space-y-4">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-yellow-300 via-yellow-100 to-yellow-300">
            Supreme Membership Required
          </h1>

          <p className="text-sm sm:text-base text-slate-200/80 max-w-md">
            You are not a{' '}
            <span className="font-semibold text-emerald-300">Supreme Member</span>.
            Either buy the Supreme package or wait {yearsLeft} more year
            {yearsLeft === 1 ? '' : 's'} — your profile converts to Supreme after 12
            years.
          </p>
          ...
          <div className="mt-4 rounded-2xl bg-amber-500/10 border border-amber-400/60 px-4 py-3 w-full max-w-md">
            <p className="text-xs sm:text-sm text-amber-100">
              Access to the{' '}
              <span className="font-semibold text-amber-300">Supreme Store</span>{' '}
              unlocks after{' '}
              <span className="font-bold">{yearsLeft} more year{yearsLeft === 1 ? '' : 's'}</span>{' '}
              of membership or by upgrading to Supreme.
            </p>
          </div>

          <p className="mt-2 text-[11px] text-white font-semibold">
            ⭐ Your Profile will upgrade to Supreme after 12 years.
          </p>
        </div>
      </div>
    </div>
...
// Request Product Modal (unchanged)
...
const RequestProductModal: React.FC<RequestProductModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div
        className="rounded-2xl bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto scrollbar-hide"
